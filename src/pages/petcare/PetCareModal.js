...
const PetCareModal=({ open, closeModal, fetchSingle, fetchSingleImages, params })=>{
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const { latitude, longitude, petServiceType, applicablePetList  } = res.data;
				creatorUser['fullName'] = creatorUser['firstname'] + " " + creatorUser['lastname'];

				setData({ latitude, longitude, description, tags, city, header, createDate, creatorUser, petServiceType, applicablePetList  });
			} catch (error) {
				setError(error);
				closeModal();
			}
			...
			<ModalBody>
				<ApiLoader loading={dataLoading || imagesLoading}>
					<Row className={'mrb-medium'}>
						<UserBlock data={{...data.creatorUser, leftData:PetServiceOptionsMap[data.petServiceType]}} />
					</Row>
					<Row>
