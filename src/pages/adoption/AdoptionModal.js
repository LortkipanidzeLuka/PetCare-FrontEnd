...
const AdoptionModal=({open, closeModal, fetchSingle, fetchSingleImages, params})=>{
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				setData({ latitude, longitude, description, tags, city, header, createDate, creatorUser, petInfo });
			} catch (error) {
				setError(error);
				closeModal();
			}
			...
			<ModalBody>
				<ApiLoader loading={dataLoading || imagesLoading}>
					<Row className={'mrb-medium'}>
						<UserBlock data={data.creatorUser} />
					</Row>
					<Row>
